...
接下来，扫描整个 **子网** 以查找其他设备。大多数本地网络使用IPv4，它使用四个数字，每个IP地址的值在1到255之间。您子网上的设备都使用相同的前三个数字。例如，如果您的IP地址是 `192.168.1.5` ，其他设备将使用 `192.168.1.2`、 `192.168.1.6` 和 `192.168.1.200` 等地址。要使用 `nmap` 扫描此子网，请填入字符串 `192.168.1.0/24` ，该字符串涵盖子网范围 `192.168.1.0` 到 `192.168.1.255` 。使用 `-sn` 标志对整个子网范围运行 **ping扫描**：
