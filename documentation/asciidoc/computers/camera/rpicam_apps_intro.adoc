...
在 `libcamera` 内核之下，我们提供了一个自定义流水线处理程序。libcamera 使用该层驱动 Raspberry Pi 上的传感器和图像信号处理器（ISP）。libcamera包含一系列图像处理算法（IPA），包括自动曝光/增益控制（AEC/AGC）、自动白平衡（AWB）和自动镜头阴影校正（ALSC）。
