...
BCM2835 封装上的 GPIO 连接有时在外设数据表中称为 "引脚"，这是一个半导体设计术语，意思是 "芯片与外界的连接"。
...
上电复位时，所有 GPIO 引脚均恢复为通用输入。此外，还应用了默认上拉状态，详情见 Arm 外设数据表中的备用功能表。大多数 GPIO 都应用了默认上拉状态。
...
下表给出了 BCM2835、BCM2836、BCM2837 和基于 RP3A0 的产品（如 Raspberry Pi Zero 或 Raspberry Pi 3+）的 GPIO 引脚的各种电压规格。有关计算模块的信息，请参阅 xref:compute-module.adoc#specifications[相关数据表]。
...
下表给出了基于 BCM2711 的产品（如 Raspberry Pi 4 和 Raspberry Pi 400）上 GPIO 引脚的电压规格。有关计算模块的信息，请参阅 xref:compute-module.adoc#specifications[相关数据表]。
